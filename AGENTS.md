...
- `src/utils/spotifyHandoff.ts` — the no-API path (see gotchas): `parseSpotifyPlaylistLink` turns a pasted share link into a `spotify:playlist:ID` uri (used by `PlaylistSheet.tsx`), `openInSpotify` deep-links it into the Spotify app (used by `NowPlayingContext`). Native tries `uri:play` → `uri` → https; web leads with https (Safari swallows raw schemes)
