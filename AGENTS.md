...
- DONE 2026-07-20: mic-reactive visuals (v1, Equalizer only) — expo-audio ~56.0.12 installed; app.json expo-audio plugin carries the mic permission string; `useMicLevel(active)` (src/utils/useMicLevel.ts) meters live loudness in `mixWithOthers` mode so it never interrupts Spotify, normalises dBFS→0..1, eases it, and returns available:false on web/denial → modes keep their timed animation. Setting `micReactive` (default ON) on MotionContext + Profile toggle; privacy clause added to legal.ts. Wired into EqualizerFullscreen bars only for now (timed loop preserved as the fallback). NEEDS A FRESH BUILD (native mic module → fingerprint changes, OTA won't carry it). After device confirms it reacts AND leaves Spotify playback alone, extend the same hook to SoundWave/CircularWave/etc.
- NEXT: fresh `eas build` for the mic module → Google ID verification clears → upload build, £1.99 product in Play Console, swap test_ key for goog_ key → closed test (12 testers/14 days)
