...
- Spotify quota policy (confirmed 2026-07-18): extension requests are ORGANIZATIONS-ONLY since May 2025 and require an established business w/ 250k+ MAU — unreachable at launch. Dev-mode allowlist (5 or 25 users — docs contradict themselves; check User Management tab) must cover testers. Before public launch, build the no-API fallback: Start Drive deep-links the linked playlist into the Spotify app (spotify: URI, no auth needed) for non-allowlisted users; Cruise FM stays the visual layer.
