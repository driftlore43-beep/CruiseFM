...
try {
  ...
  // The baseline has to be READABLE or this whole section is worthless: with
  // no baseline every package looks new, every newer-than-SDK version looks
  // unproven, and the report fills with false failures — which is how the
  // first CI run of this script went, because actions/checkout clones only
  // the tip and `git show <old commit>` had nothing to read. The workflow now
  // checks out full history; if the baseline is still missing, say THAT once
  // rather than crying wolf ten times.
  let proven = null;
  ...
  if (!proven) {
    fail('proven baseline', `could not read package-lock.json at ${provenCommit} — without it nothing can be compared against a build known to launch. If this is CI, the checkout needs full history (fetch-depth: 0).`);
    proven = {};
  }
...
if (QUICK) {
  ...
  try {
    // `expo-env.d.ts` is gitignored on purpose (Expo's own template does it)
    // and regenerated by `expo start`. On a fresh checkout it does not exist,
    // and without it tsc cannot see Expo's `*.css` module declarations — so
    // CI failed on two web-only imports that are perfectly fine locally.
    // Writing Expo's own one-line file is exactly what `expo start` does.
    const envTypes = path.join(ROOT, 'expo-env.d.ts');
    if (!existsSync(envTypes)) {
      writeFileSync(envTypes, '/// <reference types="expo/types" />\n');
    }
