...
const Ctx = createContext<MotionCtx>({ dataSaver: false, setDataSaver: () => {} });

export function MotionProvider({ children }: { children: ReactNode }) {
  const [dataSaver, setDS] = useState(false);

  useEffect(() => { getDataSaver().then(setDS); }, []);
  ...
  return <Ctx.Provider value={{ dataSaver, setDataSaver }}>{children}</Ctx.Provider>;
}
